...
class MainWindow:
    ...
    def _set_window_icon(self):
        """Thiết lập icon cho cửa sổ chính với chất lượng cao"""
        try:
            import os
            from PIL import Image, ImageTk
            
            # Thử sử dụng icon chất lượng cao mới trước
            high_quality_icon_path = "icon.png"
            if os.path.exists(high_quality_icon_path):
                # Load icon chất lượng cao
                image = Image.open(high_quality_icon_path)
                # Sử dụng LANCZOS cho resampling chất lượng cao
                image = image.resize((32, 32), Image.Resampling.LANCZOS)
                
                # Đảm bảo mode RGBA cho transparency
                if image.mode != 'RGBA':
                    image = image.convert('RGBA')
                
                self.window_icon = ImageTk.PhotoImage(image)
                self.root.iconphoto(True, self.window_icon)
                print(f"✅ Sử dụng icon chất lượng cao: {high_quality_icon_path}")
                return
            
            # Fallback về icon cũ nếu không có icon mới
            fallback_icon_path = "icon.ico"
            if os.path.exists(fallback_icon_path):
                ...
                self.window_icon = ImageTk.PhotoImage(image)
                self.root.iconphoto(True, self.window_icon)
                print(f"✅ Sử dụng icon fallback: {fallback_icon_path}")
                return
                
        except Exception as e:
            print(f"⚠️ Lỗi khi thiết lập icon cửa sổ: {e}")
    ...
    def _create_header(self):
        """Tạo header với thông tin trạng thái sử dụng logo và icons hiện đại"""
        ...
        # Left side - Logo + App title và status
        left_frame = tk.Frame(header_content, bg=ModernStyle.WHITE)
        left_frame.pack(side=tk.LEFT, fill=tk.Y)
        
        # Container cho logo và title
        title_container = tk.Frame(left_frame, bg=ModernStyle.WHITE)
        title_container.pack(anchor=tk.W)
        
        # Thêm logo vào header
        self._add_logo_to_header(title_container)
        ...
        title_label.pack(side=tk.LEFT, padx=(ModernStyle.SPACE_SM, 0))
    ...
    def _add_logo_to_header(self, parent):
        """Thêm logo chất lượng cao vào header"""
        try:
            import os
            from PIL import Image, ImageTk
            
            # Sử dụng logo UI chất lượng cao
            logo_path = "ui_logo_64x64.png"
            if os.path.exists(logo_path):
                # Load logo với chất lượng cao
                logo_image = Image.open(logo_path)
                
                # Resize về kích thước phù hợp cho header (40x40)
                ...
                print(f"✅ Thêm logo vào header: {logo_path} (40x40)")
                return True
            else:
            ...
            print(f"⚠️ Lỗi khi thêm logo vào header: {e}")
            return False
    ...
    def _create_system_settings(self, parent):
        ...
        info_content_text = f"""✨ Auto Text & Image v1.3.7 - Optimized Logo & Icon
...
🎨 Logo & Icon chất lượng cao mới:
• Sử dụng ảnh nguồn 1024x1024 tối ưu
• LANCZOS resampling algorithm cho độ sắc nét tối đa
• UnsharpMask filter + Enhanced contrast/sharpness
• Multiple sizes: 16x16 đến 512x512 pixels
• ICO file với 6 kích thước embedded
...
{ModernIcons.EDIT} Giao diện hiện đại với logo:
• Logo chất lượng cao hiển thị trong header (40x40)
...
🔧 Kỹ thuật tối ưu ảnh:
• Enhanced sharpness: UnsharpMask(radius=1.0, percent=120%)
• Contrast boost: +10% | Sharpness: +20% | Brightness: +5%
• Smart cropping: Center-crop về hình vuông trước resize
• PNG optimization với compress_level=1 (chất lượng tối đa)
...
• Font SVN Poppins + Logo chất lượng cao được load tự động"""
    ...
    def _create_footer(self):
        ...
        # Right side - App info với version và logo info
        right_frame = tk.Frame(footer_content, bg=ModernStyle.WHITE)
        right_frame.pack(side=tk.RIGHT)
        
        # App version với logo
        version_label = tk.Label(right_frame, text=f"✨ Auto Text & Image v1.3.7 - Optimized Logo + Search {ModernIcons.WINDOW}",
                font=ModernStyle.get_font(ModernStyle.FONT_SIZE_SMALL, TypographyScale.MEDIUM),
                bg=ModernStyle.WHITE, fg=ModernStyle.GRAY_500)
        version_label.pack(anchor=tk.E)
        
        # Logo quality info
        logo_info_label = tk.Label(right_frame, text=f"🎨 High-Quality Logo + LANCZOS Resampling",
                font=ModernStyle.get_font(ModernStyle.FONT_SIZE_MICRO, TypographyScale.MEDIUM),
                bg=ModernStyle.WHITE, fg=ModernStyle.GRAY_400)
        logo_info_label.pack(anchor=tk.E)
